...
def run_network(inputs, viewdirs, aud_para, fn, embed_fn, embeddirs_fn, netchunk=1024*64):
    ...
    """
    inputs_flat = torch.reshape(inputs, [-1, inputs.shape[-1]]) # [1024,64,3] --> [65536,3]
    embedded = embed_fn(inputs_flat) # [65536,63]
    aud = aud_para.unsqueeze(0).expand(inputs_flat.shape[0], -1) # [64] -> [65536,64]
    embedded = torch.cat((embedded, aud), -1) # [65536,127]
    if viewdirs is not None:
        input_dirs = viewdirs[:, None].expand(inputs.shape)
        input_dirs_flat = torch.reshape(input_dirs, [-1, input_dirs.shape[-1]])
        embedded_dirs = embeddirs_fn(input_dirs_flat)
        embedded = torch.cat([embedded, embedded_dirs], -1) # [65536,154]
...
def create_nerf(args):
    ...
    def network_query_fn(inputs, viewdirs, aud_para, network_fn): \
        return run_network(inputs, viewdirs, aud_para, network_fn,
                           embed_fn=embed_fn, embeddirs_fn=embeddirs_fn, netchunk=args.netchunk_per_gpu*args.n_gpus)
...
def config_parser():
    ...
    parser.add_argument("--chunk", type=int, default=4, #1024,
    ...
    parser.add_argument("--dim_aud", type=int, default=63, #4,
...
def train():
    ...
    AudNet = nn.DataParallel(AudNet).to(device)
    AudAttNet = AudioAttNet()
    AudAttNet = nn.DataParallel(AudAttNet).to(device)
    ...
    use_batching = not args.no_batching #False!!
